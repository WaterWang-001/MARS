...
class Platform:
    ...
    def __init__(
        ...
        self.db, self.db_cursor = create_db(db_path=self.db_path, attitude_metrics=attitude_metrics)
        ...
        # [已移除] _load_interventions_from_csv 调用
        # 干预信息现在完全依赖 DB 查询
    ...
    async def refresh(self, agent_id: int):
        ...
        try:
            ...
            return {"success": False, "error": "Clock error", "posts": [], "broadcast_messages": []}

        if self.recsys_type == RecsysType.REDDIT:
            current_time = self.sandbox_clock.time_transfer(
                datetime.now(), self.start_time)
        else:
            current_time = int_time_step

        try:
            user_id = agent_id

            # --- RecSys: 获取推荐帖子 ---
            query_all_users = "SELECT agent_id FROM user ORDER BY rowid"
            self.pl_utils._execute_db_command(query_all_users, ())
            all_agent_ids_tuples = self.db_cursor.fetchall()
            all_agent_ids = [row[0] for row in all_agent_ids_tuples]

            try:
                rec_table_user_id = all_agent_ids.index(agent_id)
            except ValueError:
                return {"success": False, "message": "Agent not found in user table map."}
            
            rec_query = "SELECT post_id FROM rec WHERE user_id = ?"
            self.pl_utils._execute_db_command(rec_query, (rec_table_user_id, ))
            ...
            if self.recsys_type != RecsysType.REDDIT:
                ...
                selected_post_ids = list(set(following_posts_ids + selected_post_ids))
            ...
            try:
                # A. 获取全局广播 (所有人可见)
                bc_query = "SELECT content FROM intervention_message WHERE time_step = ?"
                self.pl_utils._execute_db_command(bc_query, (int_time_step,))
                broadcast_rows = self.db_cursor.fetchall()
                broadcast_contents = [row[0] for row in broadcast_rows]

                # B. 获取定向收买 (仅针对当前 Agent)
                # InterventionProcessor 已经完成了筛选，这里直接查 "我的指令"
                target_query = """
                SELECT content, attitude_target, intervention_type 
                FROM agent_intervention 
                WHERE time_step = ? AND agent_id = ?
                """
                self.pl_utils._execute_db_command(target_query, (int_time_step, agent_id))
                target_rows = self.db_cursor.fetchall()
                
                for row in target_rows:
                    content_text, att_json_str, i_type = row
                    # 简单的 JSON 解析
                    try:
                        att_target = json.loads(att_json_str)
                    except:
                        att_target = {}

                    my_interventions.append({
                        "type": i_type,
                ...
                pl_log.error(f"Error querying interventions: {e}")

            # --- 最终检查 ---
            if not results_with_comments and not broadcast_contents and not my_interventions:
            ...
            return {
                ...
                "broadcast_messages": broadcast_contents,
                "intervention_instructions": my_interventions # 返回收买指令
            }
